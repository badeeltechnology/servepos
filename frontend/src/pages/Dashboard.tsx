...
export default function Dashboard() {
  ...
  const { call: getInvoiceDetail } = useFrappePostCall("servepos.api.pos_session.get_invoice_detail");
  ...
  const selectInvoice = useCallback(async (inv: any) => {
    setSelectedInvoice({ ...inv, _loading: true });
    setLoadingInvoiceDetail(true);
    try {
      const res = await getInvoiceDetail({ invoice_name: inv.name });
      setSelectedInvoice(res?.message || inv);
    } catch {
          ...
          <div className="w-full max-w-5xl max-h-[85vh] rounded-xl flex flex-col bg-white shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { setShowInvoiceSummary(false); setSelectedInvoice(null) }}
                  className="flex items-center justify-center rounded-md border border-gray-200 bg-white p-2 text-gray-500 hover:bg-gray-50">
                    ...
                    <table className="w-full text-[12px]">
                      <thead className="sticky top-0 bg-white">
                        <tr className="bg-gray-50">
                          ...
                          <th className="px-4 py-2.5 text-right text-[10px] font-semibold text-gray-500 uppercase">Total</th>
                          <th className="px-4 py-2.5 text-left text-[10px] font-semibold text-gray-500 uppercase">Payment</th>
                        ...
                        {invoiceSummaryData.map((inv, idx) => (
                          <tr key={inv.name}
                            ...
                            className={`border-t border-gray-100 cursor-pointer transition-colors ${
                              selectedInvoice?.name === inv.name ? 'bg-blue-50' : idx % 2 === 1 ? 'bg-gray-50/50' : 'hover:bg-gray-50'
                            }`}>
                            <td className="px-4 py-2.5">
                              <div className="font-medium text-gray-900">{inv.servepos_order_number || inv.name}</div>
                              <div className="text-[10px] text-gray-400 font-mono">{inv.name}</div>
                            </td>
                  ...
                  {selectedInvoice && (
                    <div className="w-1/2 overflow-auto p-5">
                      <div className="mb-4 flex items-center justify-between">
                        <div>
                          <h3 className="text-[14px] font-bold text-gray-900">{selectedInvoice.servepos_order_number || selectedInvoice.name}</h3>
                          {selectedInvoice.servepos_order_number && (
                            <p className="text-[11px] text-gray-400 font-mono">{selectedInvoice.name}</p>
                          )}
                        </div>
                        <div className="flex items-center gap-2">
                          <a href={`/app/sales-invoice/${selectedInvoice.name}`} target="_blank" rel="noopener noreferrer"
                        ...
                        <>
                          <div className="space-y-3 mb-4">
                            <div className="grid grid-cols-2 gap-3">
                              <div className="rounded-lg bg-gray-50 p-3">
                                ...
                                <p className="text-[13px] font-bold text-gray-900">{fmtCurrency(selectedInvoice.grand_total)}</p>
                              </div>
                              <div className="rounded-lg bg-gray-50 p-3">
                                <p className="text-[10px] font-medium text-gray-500 uppercase">Paid Amount</p>
                                <p className="text-[13px] font-bold text-green-600">{fmtCurrency(selectedInvoice.paid_amount)}</p>
                              </div>
                              <div className="rounded-lg bg-gray-50 p-3">
                                <p className="text-[10px] font-medium text-gray-500 uppercase">Date & Time</p>
                                <p className="text-[13px] font-semibold text-gray-900">
                                  {selectedInvoice.posting_date} {selectedInvoice.posting_time?.slice(0, 5)}
                                </p>
                              </div>
                              <div className="rounded-lg bg-gray-50 p-3">
                                <p className="text-[10px] font-medium text-gray-500 uppercase">Customer</p>
                                <p className="text-[13px] font-semibold text-gray-900">{selectedInvoice.customer_name || selectedInvoice.customer || "Walk In"}</p>
                              </div>
                              {selectedInvoice.servepos_cashier && (
                                <div className="rounded-lg bg-gray-50 p-3">
                                  <p className="text-[10px] font-medium text-gray-500 uppercase">Cashier</p>
                                  <p className="text-[13px] font-semibold text-gray-900">{selectedInvoice.servepos_cashier}</p>
                                </div>
                              )}
                              {selectedInvoice.servepos_waiter && (
                                <div className="rounded-lg bg-gray-50 p-3">
                                  <p className="text-[10px] font-medium text-gray-500 uppercase">Waiter</p>
                                  <p className="text-[13px] font-semibold text-gray-900">{selectedInvoice.servepos_waiter}</p>
                                </div>
                              )}
                              {selectedInvoice.servepos_order_type && (
                                <div className="rounded-lg bg-gray-50 p-3">
                                  <p className="text-[10px] font-medium text-gray-500 uppercase">Order Type</p>
                                  <p className="text-[13px] font-semibold text-gray-900">{selectedInvoice.servepos_order_type}</p>
                                </div>
                              )}
                              {selectedInvoice.servepos_table && (
                                <div className="rounded-lg bg-gray-50 p-3">
                                  <p className="text-[10px] font-medium text-gray-500 uppercase">Table</p>
                                  <p className="text-[13px] font-semibold text-gray-900">{selectedInvoice.servepos_table}</p>
                                </div>
                              )}
                            </div>

                            {(selectedInvoice.change_amount > 0 || selectedInvoice.servepos_tip > 0 || selectedInvoice.servepos_discount_reason) && (
                              <div className="grid grid-cols-2 gap-3">
                                {selectedInvoice.change_amount > 0 && (
                                  <div className="rounded-lg bg-amber-50 border border-amber-200 p-3">
                                    <p className="text-[10px] font-medium text-amber-600 uppercase">Change</p>
                                    <p className="text-[13px] font-bold text-amber-700">{fmtCurrency(selectedInvoice.change_amount)}</p>
                                  </div>
                                )}
                                {selectedInvoice.servepos_tip > 0 && (
                                  <div className="rounded-lg bg-green-50 border border-green-200 p-3">
                                    <p className="text-[10px] font-medium text-green-600 uppercase">Tip</p>
                                    <p className="text-[13px] font-bold text-green-700">{fmtCurrency(selectedInvoice.servepos_tip)}</p>
                                  </div>
                                )}
                                {selectedInvoice.servepos_discount_reason && (
                                  <div className="rounded-lg bg-purple-50 border border-purple-200 p-3 col-span-2">
                                    <p className="text-[10px] font-medium text-purple-600 uppercase">Discount Reason</p>
                                    <p className="text-[13px] font-semibold text-purple-700">{selectedInvoice.servepos_discount_reason}</p>
                                  </div>
                                )}
                              </div>
                            )}
                            ...
                            {selectedInvoice.remarks && (
                              <div className="rounded-lg bg-blue-50 border border-blue-200 p-3">
                                <p className="text-[10px] font-medium text-blue-600 uppercase mb-1">Notes</p>
                                <p className="text-[13px] text-blue-800">{selectedInvoice.remarks}</p>
                              </div>
                            )}
                          </div>
                                  ...
                                  {(selectedInvoice.items || []).map((item: any, i: number) => (
                                    <tr key={i} className="border-t border-gray-50">
                                      <td className="px-4 py-2">
                                        <div className="text-gray-700">{item.item_name || item.item_code}</div>
                                        {item.servepos_modifiers && (() => {
                                          try {
                                            const mods = JSON.parse(item.servepos_modifiers);
                                            return mods.length > 0 ? (
                                              <div className="text-[10px] text-gray-400 mt-0.5">
                                                {mods.map((m: any) => m.name || m.modifier_name).join(", ")}
                                              </div>
                                            ) : null;
                                          } catch { return null; }
                                        })()}
                                        {item.servepos_comment && (
                                          <div className="text-[10px] text-amber-500 mt-0.5">{item.servepos_comment}</div>
                                        )}
                                      </td>
                                      ...
                                      <td className="px-4 py-2 text-right font-semibold text-gray-900">{fmtCurrency(item.amount || 0)}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                          ...
                          {(selectedInvoice.payments || []).length > 0 && (
                            <div className="rounded-lg border border-gray-200 overflow-hidden">
                              <div className="bg-gray-50 px-4 py-2">
                                <h4 className="text-[11px] font-semibold text-gray-500 uppercase">Payments</h4>
                              </div>
                              <table className="w-full text-[12px]">
                                <tbody>
                                  {(selectedInvoice.payments || []).map((p: any, i: number) => (
                                    <tr key={i} className="border-t border-gray-50">
                                      <td className="px-4 py-2 text-gray-700">{p.mode_of_payment}</td>
                                      <td className="px-4 py-2 text-right font-semibold text-green-600">{fmtCurrency(p.amount || 0)}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
            ...
            {invoiceSummaryData.length > 0 && (
              <div className="border-t border-gray-200 px-5 py-3 flex items-center justify-between bg-gray-50">
                <span className="text-[12px] text-gray-500">{invoiceSummaryData.length} invoices</span>
                <span className="text-[14px] font-bold text-gray-900">
                  Total: {fmtCurrency(invoiceSummaryData.reduce((s, i) => s + i.grand_total, 0))}
                </span>
              </div>
            )}
          </div>
